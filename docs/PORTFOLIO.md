...
| **P5** Auth & confiance | OAuth **Google/Meta** démo explicite ou réel par callback serveur (`OAUTH_DEMO=0` + clés), scrypt + migration legacy, pages légales i18n (garantie/RMA, conditions, confidentialité), sécurité UI |
...
| OAuth démo par défaut | clés et URI fournisseurs à enregistrer | activer `OAUTH_DEMO=0` uniquement après configuration et recette des callbacks réels |
