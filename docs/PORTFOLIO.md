...
| **P5** Auth & confiance | OAuth **Google/Meta** en mode démo explicite (callbacks réels prévus en phase 2), scrypt + migration legacy, pages légales i18n (garantie/RMA, conditions, confidentialité), sécurité UI |
...
4. **OAuth démo explicitement isolé.** Le consentement simulé (`OAUTH_DEMO=1`) ne peut plus être atteint quand le mode démo est désactivé. Les callbacks OAuth réels font partie de la phase 2 du plan de remédiation.
...
| OAuth démo par défaut | pas de clés fournie | callbacks réels à livrer en phase 2 ; ne pas activer `OAUTH_DEMO=0` avant |
