...
const login = await j(`${API}/api/auth/login`, {
  ...
  body: JSON.stringify({ email: 'karim.oran@demo.dz', password: 'Qw3nt9zKp7mL2jX8vNb' })
})
...
const master = await j(`${API}/api/auth/login`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ email: 'pcstar.info31@gmail.com', password: 'Czyx8f9g2jK3mWZ5R2Tn' })
})
