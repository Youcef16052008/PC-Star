...
describe('P16 (#8) — un compte client supprimé ne revient pas', () => {
  it('DELETE /api/customers/customer-karim puis relecture : toujours absent', async () => {
    const master = await login('pcstar.info31@gmail.com', 'Czyx8f9g2jK3mWZ5R2Tn')
    ...
    assert.equal(
      db.users.some((u) => u.id === 'customer-karim' || u.email === 'karim.oran@demo.dz'),
      false,
      'le compte client est revenu après suppression'
    )
...
describe('P16 (#13) — changer de mot de passe exige le mot de passe actuel', () => {
  it('token seul → 403 ; avec le mot de passe actuel → 200', async () => {
    const token = await login('amina.castors@demo.dz', 'Yx4nBst8mP3kL7jR2vWz')
    ...
    const good = await call('POST', '/api/me/password', { body: { password: 'nouveau1', current: 'Yx4nBst8mP3kL7jR2vWz' }, token })
...
describe('P16 (#14) — reset master sans mot de passe devinable', () => {
  it('corps vide → 400 (plus de `client31` par défaut)', async () => {
    const master = await login('pcstar.info31@gmail.com', 'Czyx8f9g2jK3mWZ5R2Tn')
    const empty = await call('POST', '/api/master/customers/customer-yacine/reset-password', { body: {}, token: master })
    ...
    assert.ok(await login('yacine.pc@demo.dz', 'Wz6kLm9pN3tQ8jX2cYvB'), 'le mot de passe du client a changé tout seul')

    const ok = await call('POST', '/api/master/customers/customer-yacine/reset-password', { body: { password: 'tmp-2026' }, token: master })
